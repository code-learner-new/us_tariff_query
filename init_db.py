    ...
    with open(csv_hts,"r",encoding="utf-8-sig",newline="") as f:
        ...
        for idx,row in enumerate(reader):
            ...
            try:
                ...
                success_hts +=1
                if success_hts %5000 ==0:
                    print(f"hts已导入 {success_hts} 条")
            except Exception as e:
                skip_hts +=1
                print(f"hts第{idx}行异常 hs={raw_hs}, err:{str(e)}")
    print(f"\n====hts导入完成：成功{success_hts}，跳过{skip_hts}====")
        ...
        with open(csv_301,"r",encoding="utf-8-sig",newline="") as f:
            ...
            for idx,row in enumerate(reader):
                ...
                try:
                    ...
                    skip_301 +=1
                    print(f"301第{idx}行异常 hs={raw_hs}, err:{str(e)}")
        print(f"====301导入完成：成功{success_301}，跳过{skip_301}====")
    except FileNotFoundError:
        print("⚠️警告：未找到301_china.csv，跳过301数据导入")
        ...
        with open(csv_fl,"r",encoding="utf-8-sig",newline="") as f:
            ...
            for idx,row in enumerate(reader):
                ...
                try:
                    ...
                    skip_fl +=1
                    print(f"强迫劳动第{idx}行异常 hs={raw_hs}, err:{str(e)}")
        print(f"====强迫劳动Forcelabor_2607.csv导入完成：成功{success_fl}，跳过{skip_fl}====")
    except FileNotFoundError:
        print("⚠️警告：未找到Forcelabor_2607.csv，请核对data目录文件名大小写")

    # =========【修改4、导入 section232.csv】=========
        ...
        with open(csv_232,"r",encoding="utf-8-sig",newline="") as f:
            ...
            for idx,row in enumerate(reader):
                ...
                try:
                    ...
                    skip_232 +=1
                    print(f"232第{idx}行异常 hs={raw_hs}, err:{str(e)}")
        print(f"====Section232导入完成：成功{success_232}，跳过{skip_232}====")
    except FileNotFoundError:
        print("⚠️警告：未找到section232.csv，跳过232数据导入")
